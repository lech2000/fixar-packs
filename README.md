...
Стандартный Python 3.10+, без внешних зависимостей (для проверки YAML
используется PyYAML, если он установлен; без него эта часть проверки
пропускается с явным сообщением, остальные проверки идут как обычно).
