# fixar-packs — паки для площадки ФateиксАР
                        agrigate.pro
## Что такое  ФиксАР
...
- **Доля с трат клиента.** Из оплаченных кредитов: площадке не менее 20%,
  ...
  каналу. **Доля разработчика с ответов агентов его пака в деле сегодня не
  начисляется** — разбор на примере «Бюджета семьи вдвоём» в разделе «Сколько
  это стоит» его описания.
...
4. **Каталог и договор.** Одобренная версия появляется в общем каталоге.
   Сама она никому не ставится: пользователь ставит пак в своё дело
   договором — с пределом трат, и подписывают договор все участники дела,
   чьи сведения пак увидит (поле `reads_sensitivity`). Вторая сторона
   договора — площадка; перед пользователем за пак отвечает она, автор
   отвечает перед площадкой. Страница договора —
   `https://agrigate.pro/pak/dogovor/`, образец текста —
   [`docs/dogovor-obrazec.md`](docs/dogovor-obrazec.md). Разбор на живом
